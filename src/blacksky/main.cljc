(ns blacksky.main
  "Kotodama WASM entrypoint for the Blacksky clean-room actor (L4) — Clojure port.
  ...
  facts (`blacksky.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "blacksky")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"blacksky.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "blacksky-compat" :tier tier :entities entities} 200])
...
(defn start! [] :blacksky-compat/ready)
